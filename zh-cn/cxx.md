...
编译后的程序可以使用 PHP `FFI` 扩展，但需要在运行环境中启用扩展能力，例如在 `php.ini` 中设置：
...
- FFI 头文件、动态库路径应按运行环境配置，避免依赖编译机上的绝对路径。
- `FFI\CData` 数组元素作为右值读取时应保持普通读取语义；只有作为左值写入时才会触发可写访问。
- 对性能敏感且 ABI 固定的接口，优先使用 `.stub.php` + C++ 实现方式，避免运行时 FFI 解析和动态调用开销。
