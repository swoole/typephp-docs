...
| [`#[StdVector(T)]`](std-container-parameters.md) | 具名函数或方法参数、属性 | StdVector 类型注解；PHP 类型可省略或为 `box`，不允许 `mixed` |
| [`#[StdMap(K, V)]`](std-container-parameters.md) | 具名函数或方法参数、属性 | StdMap 类型注解；PHP 类型可省略或为 `box`，不允许 `mixed` |
| [`#[StdOrderedMap(K, V)]`](std-container-parameters.md) | 具名函数或方法参数、属性 | StdOrderedMap 类型注解；PHP 类型可省略或为 `box`，不允许 `mixed` |
| [`#[StdList(T)]`](typed-arrays.md) | 具名函数或方法参数、属性 | StdList 类型注解；PHP 类型可省略或为 `array`，不允许 `mixed` |
| [`#[StdDict(K, V)]`](typed-arrays.md) | 具名函数或方法参数、属性 | StdDict 类型注解；PHP 类型可省略或为 `array`，不允许 `mixed` |
