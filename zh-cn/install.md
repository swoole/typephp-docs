...
打开 TypePHP Releases 页面。Linux 和 macOS 用户需要选择与操作系统、CPU 架构和
宿主机 PHP 版本匹配的软件包；Windows 工具包已经包含 PHP，可直接选择希望使用的
内置 PHP 8.4 或 PHP 8.5 版本：
...
示例中的版本号以 Releases 页面实际文件名为准。Linux 和 macOS 同时提供 PHP 8.4
ZTS 和 PHP 8.5 ZTS 版本，文件名记录了构建使用的完整 PHP 版本。不要混用其他
操作系统、CPU 架构、PHP 版本或 ZTS/NTS ABI 的二进制文件。Windows 同时提供内置
PHP 8.4 和 PHP 8.5 的完整工具包，每套均包含匹配的 PHP/PHPX 运行时与 SDK。
