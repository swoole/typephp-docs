TypePHP 编译器提供了三种高精度数值类型，可用于编写高精度、零开销的数值计算程序，例如科学计算、金融财务计算等场景。
...
TypePHP 编译器提供了三种高精度类型，底层基于成熟的 C/C++ 数学库，编译为本地机器码，**零运行时开销**：
...
TypePHP 编译器会自动检测超出原生类型精度的数值字面量，并自动转为对应的 Big* 类型。你**不需要手动包装**。
